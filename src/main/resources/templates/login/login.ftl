        ...
        <form method="post" action="/login">
            ...
            <div class="form-group">
                <#if userLogoutMessage??>
                    <div class="form-row form-control m-3 alert alert-success justify-content-center" role="alert">${userLogoutMessage}</div>
                </#if>
                <#if incorrectDataMessage??>
                    <div class="form-row form-control m-3 alert alert-danger justify-content-center" role="alert">${incorrectDataMessage}</div>
                </#if>
                <input name="username" type="text" class="form-control m-3" placeholder="Ім`я користувача" required>
